...
function sectionPayloadReady(
  ...
  if (
    expectedSection === "POINTS_STANDINGS" ||
    expectedSection === "POINTS_TRAJECTORIES"
  ) {
...
PerformancePage({
  ...
  async loadReview(
    ...
    try {
      ...
      if (
        ...
        // page one with the server-observed revision instead.
        if (!active()) {
          settleStale();
          return;
        }
      ...
      if (!after && fetchSeason && season && phase?.state === "READY") {
        ...
        if (phase?.revision && phase.semanticSha256 && baseSection) {
          ...
          if (format === "H2H") {
            requestedSections.push("H2H_FIXTURES");
          }
          const requests = requestedSections.map((requestedSection) =>
            getMyTournamentSeasonReviewSection(
              tournamentId,
              eventId,
              phase.phaseId,
          ...
          if (primarySection.status === "rejected")
            ...
            // Standings are the primary review. Publish them before waiting
            // for the optional trajectory/fixture projection.
            this.seasonSectionPages = {
              [baseSection]: primarySection.value,
            ...
            this.setData({
              ...
              v2Season: nextSeason,
              v2SelectedPhaseId: phase.phaseId,
              ...
              const surface = this.data.activeView;
            ...
            for (const [index, result] of auxiliaryResults.entries()) {
              if (result.status === "fulfilled") {
                ...
                if (sectionPayloadReady(result.value, requestedSection!))
      ...
      // Section reads may complete after a tournament, entry, or season switch.
      // Fence every shared-state commit, including the primary-section error
      // path; otherwise a stale rejection can overwrite the new surface's
      // successful overview.
      if (!active()) {
        settleStale();
        return;
  ...
  async loadSeasonPhase(phaseId: string, interactionId?: string) {
    ...
    try {
      ...
      let section: MyTournamentSeasonSection | null = primarySection.value;
      this.setData({
        v2SeasonSection: section,
        v2Loading: false,
        v2State: section.state,
      ...
      for (const [index, result] of auxiliaryResults.entries()) {
        ...
        if (
          ...
          sectionPayloadReady(result.value, requestedSection!)
        )
          sections.push(result.value);
      }
      this.seasonSectionPages = Object.fromEntries(
  ...
  async onV2LoadMore() {
    ...
    if (this.data.activeView === "season") {
      ...
      const reloadAfterRebind = (): boolean => {
        ...
        this.retryBySurface.gameweek = null;
        this.retryBySurface.season = null;
        if (!currentEntryId && this.data.v2Scope !== "ALL") {
          this.showEntryEmptyState();
          return true;
        }
        this.setData({
  ...
  observeReviewInteraction(
    ...
    const selector = errorVisible
      ? surface === "gameweek"
        ? "#perf-review-gameweek-error"
        : surface === "season"
          ? "#perf-review-season-error"
          : "#perf-primary-content"
      : surface === "gameweek"
        ? "#perf-review-gameweek"
        : surface === "season"
          ? "#perf-review-season"
          : "#perf-primary-content";
  ...
  selectTournament(index: number, interactionId?: string): Promise<void> {
    ...
    const selectionRequestId = this.viewRequestId;
    const selectionIsCurrent = () =>
      this.viewRequestId === selectionRequestId &&
      this.data.v2SelectedTournament?.tournamentId === selected.tournamentId;
    const failSelectionInteraction = () => {
      const tracker = getCurrentPagePerformanceTracker();
      if (interactionId && tracker?.hasPendingInteraction(interactionId)) {
        tracker.completeInteraction(interactionId, "failed", false);
      }
    };
    const renderSelection = new Promise<void>((resolve) => {
      this.setData({
      ...
      emptyState: eventId ? "" : "view",
        ...
        if (!selectionIsCurrent()) {
          failSelectionInteraction();
          resolve();
          return;
        }
        if (!eventId) {
          this.observeReviewInteraction(interactionId, "primary", false);
        }
        resolve();
      });
    });
    if (eventId) {
      return renderSelection.then(() => {
        if (!selectionIsCurrent()) {
          failSelectionInteraction();
          return;
        }
  ...
  onTournamentChange(event: WechatMiniprogram.PickerChange) {
    const interactionId = getPageInteractionToken(
      this,
      "onTournamentChange",
    )?.interactionId;
    return this.selectTournament(Number(event.detail.value), interactionId);
  },

  onGwChange(event: WechatMiniprogram.PickerChange) {
    const interactionId = getPageInteractionToken(this, "onGwChange")?.interactionId;
