...
test("Live landing resumes an unresolved context after hide/show", () => {
  ...
  assert.match(page, /async onShow\(\)[\s\S]*const resumed = this\.hasShown[\s\S]*if \(!resumed\) return undefined[\s\S]*await waitForAuthoritativeFollow\(\)[\s\S]*shouldReloadLiveIndex\([\s\S]*loadContext\("page-show"\)/);
