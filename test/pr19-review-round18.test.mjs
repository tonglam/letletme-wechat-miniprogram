...
test("Live Tournament captures the cold trace before auth and stops hidden continuation", () => {
  ...
  assert.match(page, /async initializeFromContext[\s\S]*const startupGeneration = \+\+this\.startupGeneration[\s\S]*await waitForAuthoritativeFollow\(\)[\s\S]*this\.startupGeneration !== startupGeneration/);
