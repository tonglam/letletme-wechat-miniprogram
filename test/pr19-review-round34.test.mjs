...
test("Live Match hidden pull refresh is owned and resumed", () => {
  ...
  assert.match(
    page,
    /onHide\(\)[\s\S]*resumeForcedRefreshAfterShow = this\.forcedRefreshPending[\s\S]*resumeLoadAfterShow = this\.resumeLoadAfterShow[\s\S]*\|\| \(!this\.resumeForcedRefreshAfterShow[\s\S]*this\.refreshContextPending[\s\S]*Boolean\(this\.liveRequest && !this\.data\.hasData\)/
  );
