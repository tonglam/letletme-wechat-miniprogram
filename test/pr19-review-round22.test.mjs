...
test("Selections resumes interrupted stats without resetting picker context", () => {
  ...
  assert.match(selections, /if \(resumeStage === "stats"\)[\s\S]*loadStats\(trace\)[\s\S]*if \(resumeStage === "tournaments"\)[\s\S]*loadTournaments\(false, trace\)[\s\S]*initializePage\(trace\)/);
