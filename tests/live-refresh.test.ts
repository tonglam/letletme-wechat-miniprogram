...
assertEqual(
  ...
  true,
  "scheduled pre-kickoff snapshots keep polling",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: true,
...
assertEqual(
  ...
  false,
  "settled snapshots stop polling",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: false,
...
assertEqual(
  ...
  true,
  "rolling rollout without metadata remains fresh",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: true,
  ...
  true,
  "stale metadata does not wedge current-event recovery",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: true,
  ...
  true,
  "preseason context keeps a low-frequency anchor probe",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: true,
    currentEventId: 33,
    selectedEventId: 33,
    snapshot: snapshot("FINALIZED"),
    windowState: "BETWEEN_GAMEWEEKS",
    nextRefreshAt: new Date(Date.now() + 60_000).toISOString(),
  }),
  true,
  "final anchors use the server refresh deadline to discover kickoff",
);
assertEqual(
  shouldPollLiveSnapshot({
    pageVisible: true,
  ...
  false,
  "offseason without an anchor does not poll",
);
assertEqual(
  shouldRevalidateCachedLiveSnapshot({
  ...
  true,
  "cached current-event payloads revalidate immediately",
);
assertEqual(
  shouldRevalidateCachedLiveSnapshot({
  ...
  false,
  "fresh network payloads do not add a metadata request",
);
assertEqual(
  shouldRevalidateCachedLiveSnapshot({
    servedStoredAt: Date.now() - 1_000,
  ...
  false,
  "cached historical payloads do not poll",
);
assertEqual(
  shouldRevalidateCachedLiveSnapshot({
    servedStoredAt: Date.now() - 1_000,
  ...
  false,
  "cached payloads wait until a hidden page is shown",
);
assertEqual(
